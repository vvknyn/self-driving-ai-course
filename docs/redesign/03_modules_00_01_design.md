...
Module 01 will receive the same **instruct density** (theory → demo → check → fill) in a later pass. This redesign does **not** rewrite `notebooks/01_cameras_and_ipm.ipynb`.
...
- Notebook is dense: principle → theory beat → demo → check → fill → test → artifact
