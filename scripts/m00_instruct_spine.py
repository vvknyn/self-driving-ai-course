...
def build_m00_cells(
    ...
    return [
        ...
        _md(
            ...
            "**Why — ReLU, not another linear layer.** Two linear maps compose into one: \\(W_2(W_1 x) = (W_2 W_1)x\\). "
        ...
        _md(
            ...
            "After the net is sure about most of the road, those easy road crops still each pay full \\(-\\log p_t\\), and there are many of them. "
            "Rarity and difficulty are different. Weights fix rarity. They leave difficulty alone.\n\n"
            "**Why — focal loss, and why the factor is \\((1-p_t)^\\gamma\\).** Multiply \\(-\\log p_t\\) by a term near 0 when \\(p_t\\) is near 1 and near 1 when \\(p_t\\) is small, so easy correct crops drop out of the gradient while hard or wrong crops keep their cross-entropy. "
            "\\(\\gamma = 0\\) forces that factor to 1, which recovers plain cross-entropy and gives you a test. "
            "\\(\\gamma = 2\\) is sharp enough that \\(p_t = 0.99\\) almost vanishes and \\(p_t = 0.1\\) barely shrinks. "
            "\\(\\alpha\\) is an optional class weight on top, for when you want both rarity and difficulty. "
