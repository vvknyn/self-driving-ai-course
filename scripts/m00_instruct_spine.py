...
def build_m00_cells(
    ...
    return [
        ...
        _md(
            ...
            "**Why — these three, before the API.** The function, the loss, and the inspection are the whole week. "
            "Softmax, ReLU, convolution, and focal loss are shapes those three ideas take, not a separate subject. "
            "If you only remember function names, you can train a net and still certify it with accuracy on a road-heavy log. "
            "The fills exist so you apply the loss and the inspection yourself, in code, on these crops."
        ),
        _md("## 0. Cold open"),
        _md(
            "### Theory\n\n"
            ...
            "**Why — start with a working crop task.** A full stack (cameras, bird's-eye view, planning) hides the one skill "
            "this week is about: fit a function, and notice when the number you track is the wrong one. "
            "A 64×64 crop is small enough to train here and concrete enough that a wrong class is a picture you can look at. "
            "You should see the failure before the vocabulary — the course direction is top-down, like FastAI, on this small task.\n\n"
            "**Why — four classes, and why road dominates.** These labels are the confusions that matter inside one patch: "
            "empty drivable surface, a vehicle ahead, a person, and paint on the road. "
            "Real logs are mostly empty road, because that is what the cameras see most of the time. "
            "This sample keeps that skew on purpose. A balanced toy would hide the failure that matters: "
            "a model that looks accurate and still misses pedestrians.\n\n"
            "**Why — the untrained net and the always-road rule.** Random weights have not seen a label, so argmax is noise. "
            "That is the baseline \"nothing has been learned.\" The always-road rule is the other baseline: no network, just the majority class. "
            "It can score well on accuracy and still have pedestrian recall of zero. "
            "Later beats exist to beat both of those, not to decorate a training loop."
        ),
        _code(
            "import sys\nfrom pathlib import Path\nimport torch\nimport matplotlib\n"
        ...
        _md(
            ...
            "**Why — an image is an array, not a photo.** The net never sees \"a pedestrian.\" It sees a table of numbers. "
            "If a crop looks like a person to you and like a similar blob of pixels to the arithmetic, the model can only learn the arithmetic. "
            "Plotting one channel as numbers is how you check that the file you think you loaded is the tensor you actually train on.\n\n"
            "**Why — divide by 255, and stop there.** Raw values near 255 make the first weighted sums large, so a modest learning rate takes a huge step "
            "and the loss can diverge. Dividing by 255 is a fixed change of units, the same for every crop, so it does not teach the class. "
            "Subtracting a dataset mean and dividing by standard deviation is a later refinement. This week the requirement is a shared, bounded scale.\n\n"
            "**Why — channels before height and width.** A convolution dots a kernel with a neighborhood and writes one number per output channel. "
            "PyTorch stores those channels on axis 1 so the kernel's input-channel count lines up with the tensor. "
            "Height-width-channel is how files and screens are stored. Leave a batch that way and Conv2d treats height as if it were the number of colors. "
            "The leading B is how many crops share one step, not a spatial axis. Separate R, G, and B matter on the road: a lane dash and a shirt can share brightness "
            "and differ in color. Collapsing to gray would throw that away before learning starts."
        ),
        _md("### Demo"),
        _code(
        ...
        _md(
            ...
            "Class 0 has the largest logit, so it gets the largest pₖ. "
            "CE is small when p_y is near 1 and grows without bound as p_y goes to 0, because log p goes to −∞.\n\n"
            ...
            "**Why — scores during learning, a class name only at the end.** At decision time, argmax can say \"pedestrian.\" "
            "During learning, a hard label throws away how wrong you were. A true-class logit of 0.1 and a true-class logit of 8 can share an argmax "
            "and must not share an update. Scores keep a magnitude the loss can push.\n\n"
            "**Why — softmax, not argmax alone.** Argmax is a decision. Softmax is the comparison that turns any real scores into positive weights summing to 1, "
            "so \"negative log probability of the true class\" is defined. Without the sum-to-one step, a model can raise every logit and look more confident "
            "without preferring the true class. Softmax forces a trade: probability spent on road is probability not spent on the pedestrian. "
            "That is the right constraint when the crop has one label.\n\n"
            "**Why — cross-entropy, not a 0–1 grade.** A right-or-wrong loss is flat almost everywhere, so there is no gradient until the chosen class flips. "
            "Negative log grows without bound as the true-class probability goes to 0, and it is near 0 when that probability is near 1. "
            "A crop labeled pedestrian that the model calls road with probability 0.99 produces a large charge. A crop it already gets right produces a small one. "
            "The batch average is why one very wrong crop can still move the weights."
        ),
        _md("### Demo"),
        _code(
        ...
        _md(
            ...
            "z = w · x + b,    y = ReLU(z),    ReLU(z) = max(0, z)\n\n"
            "A **layer** is many neurons. With PyTorch `nn.Linear`, batch shape is **(B, F_in)** and "
            "weights have shape **(F_out, F_in)** so Y = ReLU(X Wᵀ + b).\n\n"
            "This beat is not yet an image model; images come once you trust the train loop on a toy case.\n\n"
            "**Why — a weighted sum plus a bias.** A score has to depend on the features, and different features count differently. "
            "In a crop, a vertical edge is not the same evidence as a bright patch. The weights are those importances. "
            "The bias is the score you would emit before seeing the features, a tilt. Together they are the smallest map that can rank one pattern above another.\n\n"
            "**Why — ReLU, not another linear layer.** Two linear maps compose into one: W₂(W₁x) = (W₂ W₁)x. "
            "Depth without a bend does not add functions. You still have one flat cut through the input. "
            "ReLU zeros the negative side, so different regions of the input activate different weights. "
            "XOR on four corners is the small proof: one line cannot separate them, and a hidden layer with a bend can. "
            "\"Pedestrian\" in pixel space is a harder version of that fact. It is not one half-plane of the crop.\n\n"
            "**Why — a 2-feature toy before the image net.** If the bend fails on four points, it will not succeed on 64×64×3. "
            "The toy keeps the failure on one plot. Batch norm inside DrivingClassifier rescales activations, and it is not a substitute for this point. "
            "Without a nonlinearity, stacking later layers would still be one linear function of the pixels."
        ),
        _md("### Demo"),
        _code(
        ...
        _md(
            ...
            "3. **Backward:** `loss.backward()` fills each parameter's `.grad` with ∂loss/∂param. "
            ...
            "**Why — those four steps, in that order.** The forward pass is the function you currently have. "
            "The loss turns a wrong class into one number the optimizer can see. Backward asks how that number would change if each weight moved a little. "
            "The step moves the weights. Skip the loss and there is no target. Skip backward and the step is a guess. "
            "Run them out of order and you apply yesterday's gradient to today's batch.\n\n"
            "**Why — a batch, not one crop and not the whole log.** One crop is a noisy opinion about the weights. "
            "That pedestrian may be dark, partly hidden, or oddly framed. A batch averages a few of those opinions so the step is less twitchy. "
            "The full training set would be a calmer average and a much slower one: you would wait for every image before a single update. "
            "Sixteen crops is the compromise here. An epoch is one pass so every training crop gets a turn. "
            "The validation pass must not call `step`, or you would fit the split you use to judge the model.\n\n"
            "**Why — zero the gradient.** `backward` adds into `.grad`. It does not replace it. "
            "Forget `zero_grad` and the next batch's gradient sits on top of the previous one. "
            "The step then chases a sum of old errors. That is not a larger learning rate. It is a corrupted direction.\n\n"
            "**Why — learning rate is a distance.** The gradient says which way the loss decreases. The learning rate says how far to walk. "
            "Too small, and you barely leave the random initialization in the epochs you have. "
            "Too large, and you step past the valley: the loss climbs or becomes NaN. "
            "The scaffold treats a non-finite loss as a failed run, because the weights are no longer a function you can trust. "
            "The break demo is the same idea on a straight line, where the overshoot is visible without waiting on images."
        ),
        _md("### Demo"),
        _code(
        ...
        _md(
            ...
            "**Why — not a dense layer on every pixel.** A linear layer from 64×64×3 into even one neuron needs a separate weight per pixel. "
            "Those weights do not know that a lane dash on the left is the same kind of mark as a lane dash on the right, so you relearn it at every location, "
            "and you need far more crops to pin the weights down. Neighboring pixels are the ones that form an edge. "
            "Pixels on opposite corners of a crop are often different objects. A dense map treats those cases as unrelated numbers.\n\n"
            "**Why — a small filter that slides.** A 3×3 kernel asks a local question: does this neighborhood look like an edge, a blob, or a color change? "
            "Weight sharing asks that same question everywhere. That bias fits driving crops, where a person's outline is the same kind of pattern on the left of the patch and on the right. "
            "One kernel cannot recognize a whole person. It can only report a local pattern. Later layers combine the reports.\n\n"
            "**Why — many channels, stacked.** One filter is one pattern. The stem's 32 channels are 32 different patterns on RGB, computed together. "
            "Each of stage2's 64 channels looks at that whole set of maps, so a higher channel can mean \"vertical edge and a bright column together,\" not a second copy of the first edge. "
            "A person in a crop is not one 3×3 template. The class scores need the mix.\n\n"
            "**Why — stride, then a pool that forgets location.** Stride 2 spends the same kind of filter on a coarser grid, so the next layer sees a wider piece of the crop without a giant kernel. "
            "Adaptive average pool to 1×1 then throws the grid away and keeps one number per channel. That matches the question \"what is this crop,\" because the label has no position inside the patch. "
            "It is the wrong last step when the question is \"where is the lane.\" We are not solving location this week. The pool is a choice of question."
        ),
        _md("### Demo"),
        _code(
        ...
        _md(
            ...
            "Diagonal entries are correct; off-diagonal cells are error types.\n\n"
            ...
            "**Why — accuracy under imbalance.** Accuracy counts every crop the same. If most crops are clear road, getting the rare pedestrian wrong still looks like a good score. "
            "The always-road rule is the extreme: no model, only the majority class, accuracy well above a coin flip on this set, pedestrian recall exactly zero. "
            "A driving log has the same skew. A metric that rewards the common class will certify a system that misses the uncommon one.\n\n"
            "**Why — read the matrix, not only the diagonal.** One recall number says people were missed. It does not say what they were called. "
            "Rows are the true class and columns are the decision, so pedestrian-called-road is a different mistake from pedestrian-called-vehicle. "
            "Those two mistakes ask for different next labels. The diagonal is the part accuracy already told you.\n\n"
            "**Why — recall for the pedestrian.** Recall asks: of the people who were there, how many did you find? "
            "Missing a pedestrian is the failure this module refuses to hide behind a single accuracy. "
            "Support sits in the denominator, which is why a class with a handful of people can look fine in accuracy and terrible in recall. "
            "`minority_recall` is that ratio, and you will write it.\n\n"
            "**Why — precision is the other mistake.** Precision asks: of the crops you called pedestrian, how many were? "
            "A model that labels every crop pedestrian has perfect recall and useless precision. The car would brake for lane paint. "
            "Driving needs both. The trap you are graded on this week is the miss, because accuracy already hides it. "
            "Do not ignore precision when you read the matrix."
        ),
        _md("### Demo"),
        _code(
        ...
        _md(
            ...
            "We do not implement weighted CE this week — it sits between plain CE and focal loss.\n\n"
            "**Focal loss** (Lin et al.) multiplies CE by a **focusing factor** that depends on how correct the model already is:\n\n"
            "pₜ = p_y,    FL = −αₜ (1 − pₜ)^γ log(pₜ)\n\n"
            "Derive the factor, do not only memorize the name:\n\n"
            "- If γ = 0, (1 − pₜ)⁰ = 1, so FL matches CE when α is absent.\n"
            "- If the model is confident and **correct**, pₜ → 1 and (1 − pₜ)^γ → 0 — the example stops dominating the gradient.\n"
            "- If the model is unsure or **wrong**, pₜ is small and (1 − pₜ)^γ ≈ 1 — CE remains.\n"
            "- γ = 2 is the default in `TrainConfig.gamma`.\n"
            "- αₜ is an optional per-class weight, shape `(K,)`, applied as `alpha[targets]`; `None` means no extra class weight.\n\n"
            "Focal loss does not replace inspection (Principle 3) or good labels — it changes which mistakes the step listens to.\n\n"
            "**Why — the loss is what \"good\" means to the step.** The optimizer never sees your dashboard. It sees the scalar you differentiated. "
            "If that scalar is content with easy road, the weights move to serve easy road. "
            "Choosing plain cross-entropy, a class-weighted variant, or focal loss is choosing which crops may dominate the update.\n\n"
            "**Why — a class weight is only half the problem.** A weight per class makes each pedestrian crop cost more than each road crop. "
            "That matches rarity. It does not look at whether the model already got the crop right. "
            "After the net is sure about most of the road, those easy road crops still each pay full −log pₜ, and there are many of them. "
            "Rarity and difficulty are different. Weights fix rarity. They leave difficulty alone.\n\n"
            "**Why — focal loss, and why the factor is (1 − pₜ)^γ.** Multiply −log pₜ by a term near 0 when pₜ is near 1 and near 1 when pₜ is small, so easy correct crops drop out of the gradient while hard or wrong crops keep their cross-entropy. "
            "γ = 0 forces that factor to 1, which recovers plain cross-entropy and gives you a test. "
            "γ = 2 is sharp enough that pₜ = 0.99 almost vanishes and pₜ = 0.1 barely shrinks. "
            "α is an optional class weight on top, for when you want both rarity and difficulty. "
        ...
        _md(
            "### Check\n\n"
            "From the focal demo table (γ = 2): at pₜ = 0.99, is (1 − pₜ)² below 0.001? "
            "At pₜ = 0.10, is the factor above 0.5? The demo cell above printed the table and asserted both."
        ),
        _md(
        ...
        _md(
            ...
            "**Why — look at the mistakes, not another average.** Recall can rise while the errors that remain are the confident ones: the model is sure a person is road. "
            "Those are the crops that matter on a road, and the crops a labeling loop should queue. An average cannot show you the picture. A gallery can.\n\n"
            "**Why — sort by confidence in the wrong class.** A low-confidence mistake is the model shrugging. A high probability on the wrong class is the model insisting. "
            "Insisting is the dangerous case, and it is the case you would send for another label. Sorting that way is the small form of hard-example mining. "
            "A fleet data engine is one large version of the same sort, not a different idea.\n\n"
            "**Why — a rate you compute, a test, and a file.** `minority_recall` is the scalar the gallery does not replace: you still need the rate, and you write the ratio so the definition is yours. "
            "The gallery is from scratch because the point is the sort, not a library call. "
            "Pytest checks contracts you can get wrong while the pictures still look plausible. "
            "The break-it script repeats the always-road trap outside this notebook, so the metric lesson is not trapped in one session. "
            "The artifact is the run you can open tomorrow; opening the notebook is not the work."
        ),
        _md(
        ...
        _md(
            ...
            "Why does γ = 0 match cross-entropy? Write 3–6 sentences before opening `solutions/00_ml_gym`."
        ),
